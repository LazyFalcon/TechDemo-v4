...
void Pathfinder::saveVecAsImage(std::vector<short> vec2, std::string name) {
    ...
    for(int i = 0; i < vec.size(); i++) { vec[i] = (255 * 255) * (vec2[i] - min) / (max - min); }
...
const std::vector<glm::ivec2> nextMoves {{0, 2}, {2, 2}, {2, 0}, {2, -2}, {0, -2}, {-2, -2}, {-2, 0}, {-2, 2}};
...
Waypoint Pathfinder::getNextBestField(Waypoint waypoint) {
    ...
    Waypoint pp;
    pp.position =
        glm::vec4(bestPoint.x - mapSize / 2, bestPoint.y - mapSize / 2, heightField.get(bestPoint) + 10.5f, 1);
...
float Pathfinder::calculateFieldValue(glm::ivec2 position) {
    ...
    for(auto& a : m_scene.getHostiles()) { value += -0.05f / (glm::distance(a->eq().getPosition(), p) + 1); }

    for(auto a : semiStaticObjects) { value += a.value / a.currentDistance(position); }
...
void Pathfinder::preprocessMap() {
    ...
    for(int i = 0; i < size; i++) {
        simplified[i] = 255
                        * ((dataBeingProcessed.heightmap[i] - dataBeingProcessed.min)
                           / (dataBeingProcessed.max - dataBeingProcessed.min));
    }
