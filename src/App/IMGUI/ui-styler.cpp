...
void Styler::render(Panel& panel) {
    ...
    if(panel.m_background.color)
        m_renderedUiItems.put<RenderedUIItems::Background>(panel.m_background);
    if(panel.m_blured)
        m_renderedUiItems.put<RenderedUIItems::ToBlur>(
            RenderedUIItems::ToBlur {panel.m_background.box, panel.m_background.depth});
}
void Styler::render(Item& item) {
    u32 itemColor = 0xf0f0f040;
    if(item.m_action > PointerActions::Hover and item.m_action <= PointerActions::RmbHold)
        itemColor = 0xf0f0f0a0;
    else if(item.m_action == PointerActions::Hover)
        itemColor = 0xf0f0f080;
