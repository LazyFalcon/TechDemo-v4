...
bool Input::execute(int k, int a, int m, u32 currentTime) {
    if(m_forwardAction)
        m_forwardAction(convertKeyToString(k, a, m));

    auto range = actions.equal_range(hashInput(k, a, m));
    if(range.second == range.first)
        range = actions.equal_range(hashInput(k, a, 0));
    ...
    for(auto it = range.first; it != range.second; ++it) {
        ...
        if(a == GLFW_REPEAT) {
            if(it->second.lastTimeUsed == 0)
                it->second.lastTimeUsed = currentTime; // * case when we dont have .on set
            currentKey.onHoldTime = timeDifferenceFormLastAction;
        }
        else {
            it->second.noOfRepeats =
                (timeDifferenceFormLastAction < repeatTresholdTime) ? it->second.noOfRepeats + 1 : 0;
...
bool Input::executeTwoArgs(int arg, float x, float y) {
    ...
    for(auto it = range.first; it != range.second; ++it) { it->second(x, y); }
    return false;
}
...
void Input::startHoldKey(int k, int m, u32 currentTime) {
    ...
    if(range.second == range.first)
        range = actions.equal_range(hashInput(k, GLFW_REPEAT, 0));
    for(auto it = range.first; it != range.second; ++it) { it->second.lastTimeUsed = currentTime; }
}
void Input::releaseHeldKey(int k, int m) {
    auto range = actions.equal_range(hashInput(k, GLFW_REPEAT, m));
    if(range.second == range.first)
        range = actions.equal_range(hashInput(k, GLFW_REPEAT, 0));
    for(auto it = range.first; it != range.second; ++it) { it->second.lastTimeUsed = 0; }
}
