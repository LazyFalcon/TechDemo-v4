...
public:
    ...
    glm::vec4 operator()(u32 x, u32 y) {
        return {data[x + y * width] / 255.f, 0, 0, 0};
    }
    ...
    if(settings & LINEAR) {
        ...
        if(settings & CLAMP) {
            i = {
                x,
                y,
                glm::clamp(x + 1, 0u, width),
                y,
                x,
                glm::clamp(y + 1, 0u, height),
                glm::clamp(x + 1, 0u, width),
                glm::clamp(y + 1, 0u, height),
            };
        }
            ...
            i = {
                x, y, (x + 1) % width, y, x, (y + 1) % height, (x + 1) % width, (y + 1) % height,
            };
        }
        return pmk::bilinearInterpolate((*data)(i[0], i[1]), (*data)(i[2], i[3]), (*data)(i[4], i[5]),
                                        (*data)(i[6], i[7]), u, v);
    }
