...
SunCoordinates calc(Time udtTime, Location udtLocation) {
    ...
    // and JD 2451545.0, which is noon 1 January 2000 Universal Time
    {
        ...
        liAux2 = (1461 * (udtTime.year + 4800 + liAux1)) / 4 + (367 * (udtTime.month - 2 - 12 * liAux1)) / 12
                 - (3 * ((udtTime.year + 4900 + liAux1) / 100)) / 4 + udtTime.day - 32075;
    ...
    // (i.e., the result may be greater than 2*Pi)
    {
        ...
        eclipticLongitude = meanLongitude + 0.03341607 * sin(meanAnomaly) + 0.00034894 * sin(2 * meanAnomaly)
    ...
    // Calculate local coordinates ( azimuth and zenith angle ) in degrees
    {
        ...
        udtSunCoordinates.zenithAngle =
            (acos(cos_Latitude * cos_HourAngle * cos(declination) + sin(declination) * sin_Latitude));
        dY = -sin(hourAngle);
