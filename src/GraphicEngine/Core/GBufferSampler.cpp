...
void GBufferSamplers::sampleGBuffer(Camera &camera){
    ...
    gl::FramebufferTexture2D(gl::READ_FRAMEBUFFER, gl::DEPTH_ATTACHMENT, gl::TEXTURE_2D, context.tex.gbuffer.depth.ID, 0);
    ...
    for(auto it : samplers){
        ...
        glm::vec4 viewSpace(screenPosition.x/window.size.x*2.f - 1.f, screenPosition.y/window.size.y*2.f - 1.f, 2*it->depth-1, 1);
    ...
    // normal
    gl::FramebufferTexture2D(gl::READ_FRAMEBUFFER, gl::COLOR_ATTACHMENT0, gl::TEXTURE_2D, context.tex.gbuffer.normals.ID, 0);
    for(auto it : samplers){
        auto screenPosition = it->relativePosition ? window.size*it->samplePosition : it->samplePosition;
        uint64_t normal;
