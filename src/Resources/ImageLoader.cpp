...
ImageParams loadImageToGpu(const std::string& filePath) {
    ...
    gl::TexImage2D(gl::TEXTURE_2D, 0, data.internalFormat, data.width, data.height, 0, data.format, data.type,
                   data.data);
...
ImageParams loadArrayToGpu(const std::vector<fs::path>& files) {
    ...
    for(u32 i = 0; i < layers; i++) { tmp[i] = loadToMemory(files[i].string(), ImageDataType::RGBA8); }
    ...
    gl::TexImage3D(gl::TEXTURE_2D_ARRAY, 0, data.internalFormat, data.width, data.height, data.layers, 0, data.format,
                   data.type, data.data);
...
ImageParams loadCubemapToGpu(const std::vector<std::string>& filePathes) {
    ...
    for(u32 i = 0; i < count; i++) { cubemapFaces[i] = loadToMemory(filePathes[i], ImageDataType::RGBA8); }
    data = cubemapFaces[0];
    data.data = nullptr;
    ...
    for(u32 face = 0; face < 6; face++)
        gl::TexImage2D(gl::TEXTURE_CUBE_MAP_POSITIVE_X + face, 0, gl::RGBA8, cubemapFaces[face].width,
                       cubemapFaces[face].height, 0, gl::BGRA, gl::UNSIGNED_BYTE, cubemapFaces[face].data);

    gl::GenerateMipmap(gl::TEXTURE_CUBE_MAP);
    gl::BindTexture(gl::TEXTURE_CUBE_MAP, 0);

    for(u32 i = 0; i < count; i++) { delete[](u8*) cubemapFaces[i].data; }
    ...
    switch(image.dataType) {
        case R8: {
            ...
            for(auto y = 0; y < image.height; y++) {
                ...
                for(auto x = 0; x < image.width; x++) { bits[x] = *((u8*)image.data + x + y * image.width); }
            }
        ...
        case R16: {
            ...
            for(auto y = 0; y < image.height; y++) {
                ...
                for(auto x = 0; x < image.width; x++) { bits[x] = *((u16*)image.data + x + y * image.width); }
            }
            return FreeImage_Save(FIF_PNG, dib, filePath.c_str(), PNG_DEFAULT);
        }
        case R32F: {
            ...
            for(auto y = 0; y < image.height; y++) {
                ...
                for(auto x = 0; x < image.width; x++) { bits[x] = *((float*)image.data + x + y * image.width); }
            }
