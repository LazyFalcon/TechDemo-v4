...
void positions(aiMesh& mesh, std::vector<T>& target, int startPosition) {
    ...
    if(count == 0)
        console.error(mesh.mName.C_Str(), " position data not definied");
...
void texcoords(aiMesh& mesh, std::vector<T>& target, int startPosition) {
    ...
    if(not mesh.HasTextureCoords(0)) {
        ...
        for(u32 to = startPosition; to < startPosition + count; to++) { target[to].uv = {0, 0, 0}; }
        return;
    }

    for(u32 from = 0, to = startPosition; from < count; from++, to++) {
        target[to].uv = {mesh.mTextureCoords[0][from].x, mesh.mTextureCoords[0][from].y,
                         mesh.mTextureCoords[0][from].z};
    }
}
template<typename T>
void normals(aiMesh& mesh, std::vector<T>& target, int startPosition) {
    ...
    if(not mesh.HasNormals())
        console.error(mesh.mName.C_Str(), "normal data not definied");
    for(u32 from = 0, to = startPosition; from < count; from++, to++) {
...
void tangents(aiMesh& mesh, std::vector<T>& target, int startPosition) {
    u32 count = mesh.mNumVertices;
    if(count == 0)
        console.error(mesh.mName.C_Str(), "tangent data not definied");
    for(u32 from = 0, to = startPosition; from < count; from++, to++) {
...
void color(aiMesh& mesh, std::vector<T>& target, int startPosition, glm::vec3 materialColor) {
    ...
    if(not mesh.HasVertexColors(0)) {
        for(u32 to = startPosition; to < startPosition + count; to++) { target[to].color = materialColor; }
...
public:
    ...
    ModelLoader& open(const std::string& filename) {
        ...
        if(scene)
            close();
        scene = importer.ReadFile(filename.c_str(), aiProcess_CalcTangentSpace | aiProcess_Triangulate
    ...
    */
    ...
    void setTextureLayer(InternalMeshInfo info, float layer) {
        for(u32 i = 0; i < info.vertexCount; i++) { rawVertexData[info.vertexStart + i].uv[2] += layer; }
    }
    void setBoneIndex(InternalMeshInfo info, float index) {
        for(u32 i = 0; i < info.vertexCount; i++) { rawVertexData[info.vertexStart + i].boneId += index; }
    }
    void moveModel(InternalMeshInfo info, glm::vec3 offset) {
        for(u32 i = 0; i < info.vertexCount; i++) { rawVertexData[info.vertexStart + i].position += offset; }
    }
    void moveModel(const Mesh& info, glm::vec3 offset) {
        for(u32 i = 0; i < info.vertexEnd; i++) { rawVertexData[info.vertexStart + i].position += offset; }
    }
    VAO build() {
        VAO vao {};
    ...
    std::vector<std::string> getNames() {
        ...
        for(int i = 0; i < scene->mNumMeshes; i++) { out.emplace_back(scene->mMeshes[i]->mName.C_Str()); }
