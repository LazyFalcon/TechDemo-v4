...
glm::vec2 splitVec2(std::string toSplit) {
    ...
    int i = 0;
    for(; beg != end; beg++) { out[i++] = (float)std::stoi(*beg); }
    return out;
}
...
void Font::load(const std::string& name, std::vector<std::string>& imagesToLoad) {
    ...
    for(int i = 0; i < charCount; i++) { getline(file, lines[i]); }
    for(auto& it : lines) { loadCharacter(it, sizeU, sizeV, pageOffset); }
    std::string kernings;
    ...
    if(kerningCount) {
        ...
        for(int i = 0; i < *kerningCount; i++) getline(file, lines[i]);
        for(auto& it : lines) loadKerning(it);
    }
    file.close();
