...
std::shared_ptr<Joint> createJoint(const Yaml& config, glm::vec4 fromSocketToOrigin){
    std::shared_ptr<Joint> out;
    if(config["Type"] == "Rigid") out = std::make_shared<Rigid>(config);
