...
DroneLikeControl::DroneLikeControl(PhysicalWorld& physics, Vehicle& eq, btVector3 position)
    : VehicleControlSystem(eq), m_physics(physics), m_virtualPosition(position), m_lookDirection(0, 1, 0) {
...
void DroneLikeControl::computeState() {
    if(vehicle.control.controlOnAxes.x == 0.f and vehicle.control.controlOnAxes.y == 0.f
       and vehicle.control.controlOnAxes.z == 0.f) {
        if(vehicle.rgBody->getLinearVelocity().length2() > 0.001f)
            state = Breaking;
        else
            state = Steady;
    }
...
btVector3 DroneLikeControl::getDesiredAngles(const btVector3& targetDir, const btVector3& normalToPlane,
                                             const btVector3& rightAxis) {
    return btVector3(angleToPlane(normalToPlane, targetDir), angleToPlane(normalToPlane, rightAxis),
...
void DroneLikeControl::orientationPart(float dt, const btTransform& tr) {
    ...
    const btVector3 targetLocalAngles(angleToPlane(locZ, m_virtualDirection), angleToPlane(btVector3(0, 0, 1), locX),
    ...
    { // control inclination, allow to small move with target direction - best is to limit it directly in target
        ...
        {                                                      // for X axis
            ...
            const float desiredSpeed =
                glm::sign(orientedAngle) * glm::smoothstep(0.f, pi / 30, abs(orientedAngle)) * maxSpeed;
    ...
    { // control velocites directly, but consider previous velocity, so there is a cap for applied velocity change
        ...
        const float desiredSpeed = glm::sign(orientedAngle) * glm::smoothstep(0.f, pi / 20, abs(orientedAngle))
                                   * maxSpeed; // smooth speed near target angle
