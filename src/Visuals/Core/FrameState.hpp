struct FrameState
{
    ...
    void bind(){
        ...
        int drawbuffers = 0;
        if(firstTexture) gl::FramebufferTexture2D(gl::DRAW_FRAMEBUFFER, gl::COLOR_ATTACHMENT0, gl::TEXTURE_2D, firstTexture->ID, 0);
        if(secondTexture) gl::FramebufferTexture2D(gl::DRAW_FRAMEBUFFER, gl::COLOR_ATTACHMENT0, gl::TEXTURE_2D, secondTexture->ID, 0);
        if(thirdTexture) gl::FramebufferTexture2D(gl::DRAW_FRAMEBUFFER, gl::COLOR_ATTACHMENT0, gl::TEXTURE_2D, thirdTexture->ID, 0);
        if(depthTexture){
