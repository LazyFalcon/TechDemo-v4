...
std::string combine(Lines& lines, int from = 0, int to = 99999) {
    ...
    for(int i = from; i < to; i++) out += lines[i] + "\n";
    ...
    auto extractExport = [&](int exportPosition) -> int {
        ...
        for(int i = exportPosition + 1; i < lines.size(); i++) {
            if(not lines[i].empty() and lines[i][0] == '@'
                   and (lines[i].find("export") != std::string::npos or lines[i].find("shader") != std::string::npos
                        or lines[i].find("end") != std::string::npos)
               or i == lines.size() - 1) {
                auto name = getNameFrom(lines[exportPosition]);
                if(name == "")
                    throw std::runtime_error("export must have name! : line: " + std::to_string(i));
...
auto splitOnShaders(Lines& lines, const std::string& defaultName) {
    ...
    auto upTo = [&](int from) -> int {
        ...
        for(; i <= lines.size(); i++) {
            if(i == lines.size()
               or (not lines[i].empty() and lines[i][0] == '@'
                   and (lines[i].find("shader") != std::string::npos or lines[i].find("end") != std::string::npos))) {
...
auto splitShader(Lines& lines) {
    ...
    for(int i = 0; i < lines.size(); i++) {
        if(not lines[i].empty() and lines[i][0] == '@') {
            ...
            if(lines[i].find("vertex") != std::string::npos)
                out.vertex = combine(lines, i + 1, to);
            else if(lines[i].find("fragment") != std::string::npos)
                out.fragment = combine(lines, i + 1, to);
            else if(lines[i].find("geometry") != std::string::npos)
                out.geometry = combine(lines, i + 1, to);
            else if(lines[i].find("tesselation") != std::string::npos)
                out.tesselation = combine(lines, i + 1, to);
            else if(lines[i].find("compute") != std::string::npos)
                out.compute = combine(lines, i + 1, to);
...
Shader compileShader(Lines& lines) {
    ...
    if(stages.vertex)
        shaderPrograms.vertex = compile(*stages.vertex, gl::VERTEX_SHADER);
    if(stages.geometry)
        shaderPrograms.geometry = compile(*stages.geometry, gl::GEOMETRY_SHADER);
    if(stages.fragment)
        shaderPrograms.fragment = compile(*stages.fragment, gl::FRAGMENT_SHADER);

    u32 shaderId = gl::CreateProgram();
    if(shaderPrograms.vertex)
        gl::AttachShader(shaderId, *shaderPrograms.vertex);
    if(shaderPrograms.geometry)
        gl::AttachShader(shaderId, *shaderPrograms.geometry);
    if(shaderPrograms.fragment)
        gl::AttachShader(shaderId, *shaderPrograms.fragment);

    gl::LinkProgram(shaderId);

    if(shaderPrograms.vertex)
        gl::DeleteShader(*shaderPrograms.vertex);
    if(shaderPrograms.geometry)
        gl::DeleteShader(*shaderPrograms.geometry);
    if(shaderPrograms.fragment)
        gl::DeleteShader(*shaderPrograms.fragment);

    if(shaderPrograms.vertex)
        gl::DetachShader(shaderId, *shaderPrograms.vertex);
    if(shaderPrograms.geometry)
        gl::DetachShader(shaderId, *shaderPrograms.geometry);
    if(shaderPrograms.fragment)
        gl::DetachShader(shaderId, *shaderPrograms.fragment);
...
auto compileShaders(const std::string& filepath, const std::string& filename) {
    ...
    if(shaders.empty())
        out[filename] = ::compileShader(content);
    else
        for(auto& it : shaders) out[it.first] = ::compileShader(it.second);
