...
enum ShadowSize {
    BigShadow, MediumShadow, SmallShadow
}
...
public:
    ...
    void allocateResources(Texture tex, std::vector<ShadowSize> requestedSizes){ // * split leci w rzędach, warto więc by było równo, nie ma skomplikowanego algorytmu podziału
    ...
    std::optional<ShadowHandle> allocateResource(){
        if(m_freeResources.empty()) return {};
        return
    }
