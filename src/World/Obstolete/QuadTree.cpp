...
void QuadTree::init(const Yaml& cfg) {
    ...
    for(int i = 0; i <= levels; i++) { numberOfChunksInAllLevels += pow(4, i); }
...
void QuadTree::buildQTNodes(QTNode& node, u32 lodLevel, Terrain& terrain) {
    ...
    if(lodLevel == 0) { // final node
        ...
        node.center.z = (node.children[0]->center.z + node.children[1]->center.z + node.children[2]->center.z
                         + node.children[3]->center.z)
                        / 4.f;
    }
}
...
void diff(nodeVec& old, nodeVec& cur, nodeVec& added, nodeVec& removed) {
    ...
    for(; oldI < oldS; oldI++) { removed.push_back(old[oldI]); }
    for(; curI < curS; curI++) { added.push_back(cur[curI]); }
}
...
void QuadTree::update(const camera::Frustum& frustum) {
    ...
    for(u32 i = 0; i < lodLevels.size(); i++) {
        ...
        for(auto& r : it.removedNodes) { r->visible = false; }

        std::sort(it.renderData.begin(), it.renderData.end(),
                  [&frustum](const glm::vec4& a, const glm::vec4& b) -> bool {
...
SampleResult QuadTree::sampleHeight(glm::vec4 position, const QTNode* node) const {
    auto findChunk = [this, &position]() -> i32 {
        ...
        while(true) {
            if(node->lodLevel == 0)
                return node->id;
            if(node->children[0]->insideFlat(position))
                node = node->children[0];
            else if(node->children[1]->insideFlat(position))
                node = node->children[1];
            else if(node->children[2]->insideFlat(position))
                node = node->children[2];
            else if(node->children[3]->insideFlat(position))
...
SampleResult QTNode::sampleHeight(glm::vec4 position) const {
    ...
    position.z = pmk::triangleInterpolate((*payload.terrainData)(x, y), (*payload.terrainData)(x, y + 1),
                                          (*payload.terrainData)(x + 1, y), (*payload.terrainData)(x + 1, y + 1), u, v);

    return {position,
            glm::vec4(pmk::triangleNormal((*payload.terrainData)(x, y), (*payload.terrainData)(x, y + 1),
                                          (*payload.terrainData)(x + 1, y), (*payload.terrainData)(x + 1, y + 1), u, v,
