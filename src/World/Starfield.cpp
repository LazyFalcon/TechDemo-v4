...
VAO Starfield::upload(std::vector<Star> &stars){
    VAO vao {};
    vao.setup();
    vao.vbo[0].setup(stars)
        .attrib(0).pointer_float(4, sizeof(Star), (void*)offsetof(Star, position)).divisor(0)
        .attrib(1).pointer_color(sizeof(Star), (void*)offsetof(Star, color)).divisor(0)
        .attrib(2).pointer_float(1, sizeof(Star), (void*)offsetof(Star, brightness)).divisor(0)
        ;
    vao.vboCount = 1;
    vao();
    return vao;
