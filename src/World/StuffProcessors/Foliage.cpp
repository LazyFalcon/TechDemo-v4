...
bool Foliage::load(const Yaml &c){
    ...
    for(const auto &tree : treesToLoad[0]){
        ...
        for(i32 i=lodCount; i<5; i++){
            trees.back().mesh[i] = trees.back().mesh[lodCount];
        }
    }
...
void Foliage::plantTrees(FoliageData &data, QTNode &node, QuadTree &QT){
    ...
    auto pushTree = [&](glm::vec4 &p){
        ...
        for(u32 i=0; i<5; i++){
            glm::vec2 force(0);
            for(auto &f : plantedTrees){
                force +=  f.w / glm::distance(cp.xy(), f.xy());
            }
    ...
    auto trim = [this](){
        for(i32 i=count-1; i>=0; --i){
            if(info[i].owner == nullptr){
                count--;
                if(info[i].rgBody) removeTreeFromPhysics(info[i].rgBody, info[i].rgJoint);
            }
            else return;
        }
    };
...
void Foliage::update(glm::vec4 reference){
    ...
    { // remove
        ...
        for(auto &it : QT.lodLevels[1].removedNodes){
            removedChunks.insert(it);
        }
        ...
        for(auto &it : removedChunks){
            if(not it->payload.foliageData) continue;
            for(auto &t : it->payload.foliageData->treeID)
                treeBatchData.info[t].owner = nullptr;
            for(auto &t : it->payload.foliageData->shrubID)
                shrubBatchData.info[t].owner = nullptr;
    ...
    { // collect added
        ...
        for(auto &it : QT.lodLevels[1].addedNodes){
            addedChunks.insert(it);
        }
...
void FoliageDensityMap::generate(){
    ...
    for(float y=0; y<sampler.size.y; y+=1){
        float a = scaled_octave_noise_2d(octaves, persistence, scale, 0, 255,
                                                                    x*seed + seedOffset,
                                                                    y*seed + seedOffset
                                                                    );
...
void FoliageDensityMap::upload(){
    if(textureId == 0){
        ...
        gl::TexImage2D(gl::TEXTURE_2D, 0, gl::RED, sampler.size.x, sampler.size.y, 0, gl::RED, gl::UNSIGNED_BYTE, sampler.data.data());
