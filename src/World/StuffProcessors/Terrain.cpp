...
void Terrain::save(const std::string& dir) {
    ...
    std::transform(map.data.begin(), map.data.end(), packedMap.begin(),
                   [this](float h) { return 0xffffu * ((h - map.min) / (map.max - map.min)); });
...
void Terrain::copyElevationData(QTNode& node) {
    ...
    for(u32 v = y; v < y + chunkNodes; v++) {
        bulletDataIterator = std::copy(map.data.begin() + v * map.nodes.x + x,
                                       map.data.begin() + v * map.nodes.y + x + chunkNodes, bulletDataIterator);
    }
}
...
std::vector<glm::vec4> Terrain::generateListOfVisibleNodes() {
    ...
    for(auto& it : qt.lodLevels) { out.insert(out.end(), it.renderData.begin(), it.renderData.end()); }
    return out;
}
