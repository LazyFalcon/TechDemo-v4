...
Sun::Sun(const Yaml &sett, GeoTimePosition &geoTimePosition)
    : m_geoTimePosition(geoTimePosition)
{
...
void Sun::calcSunPosition(){
    auto coords = sunpos::calc({// TODO: pass julian days and hour of the day, use u32
                                (int)m_geoTimePosition.date().year(),
                                (int)m_geoTimePosition.date().month(),
                                (int)m_geoTimePosition.date().day(),
                                (double)m_geoTimePosition.time().hours(),
                                (double)m_geoTimePosition.time().minutes(),
                                (double)m_geoTimePosition.time().seconds()
                            }, {
                                m_geoTimePosition.longitude,
                                m_geoTimePosition.latitude
                            });
...
glm::vec4 Sun::calcSunColor(Atmosphere &atm){
    ...
    for(int i=0; i<4; i++){
        ...
        glm::dvec4 attenuate = exp(-scatter * (invWavelength * double(atm.rayleigh*4.0*pi) + double(atm.mie*4.0*pi)) );
...
void Sun::update(Atmosphere &atm){
    ...
    direction = glm::normalize(direction*(1-glm::smoothstep(88.f, 107.f, inDeg)) + glm::vec4(0,0.1,-1,0)*glm::smoothstep(88.f, 107.f, inDeg));
}
